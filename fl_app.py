...
if LOCAL:
    ...
    ## Detectors data
    detectors_df = pd.read_csv('/home/kara/mysite/assets/detector_locations.csv', index_col='Unnamed: 0')
    ## Stars data
    stars_radec_list = np.loadtxt('/home/kara/mysite/assets/star_candidates.txt', usecols=(1, 2), delimiter=',')
    candid_names = np.loadtxt('/home/kara/mysite/assets/star_candidates.txt', usecols=0, delimiter=',', dtype=str)
...
explanation_text =  f"\n\nThe app calculates the relative time delays between selected_detectors \n" \
                    ...
                    "Checkout [**SNEWS2**](https://snews2.org/).\n" \
                    "\n Simply, select a candidate star, and available detectors. The app displays the time delays" \
                    " with respect to first detector that the flux arrives.\n"
    ...
    html.Div(
        ...
        children=[
            ...
            dcc.Checklist(id="Detectors", options=detectors_df.index, value=['SK', 'IC', 'KL', 'XENONnT'],
                          style={'color': 'white', 'font_size': '25px', 'margin-top': '15px', 'margin-bottom': '15px'},
                          inline=True),
            ...
            # download button
            html.A('Download Alert Data', id='download-link', download="snews_alert.json", href="",
                   target="_blank", style={'font_size': '22px'}),
            html.Br(),
            html.A('Download Detector Messages', id='download-link-det', download="detector_delays.json",
                   href="", target="_blank", style={'font_size': '22px'}),
            html.Br(),
            html.A(html.Img(src="./assets/snews_logo.png", width='35%'),
                   href="https://github.com/SNEWS2/SNEWS_Publishing_Tools",
                   style={'margin-top': '10px', 'margin-bottom': '10px'}, )
        ],
        style={'width': "30%", 'margin-left': '3vw', 'margin-top': '3vw'}
    ),
    # central panel
    html.Div(children=[
        ...
        style={'width': "25%", 'margin-top': '3vw'}
    ),
    # right panel
    html.Div(children=[
        dcc.Graph(id='my_3dscat', figure={})
    ],
        style={'flex': 1, 'margin-left': '1vw', 'margin-top': '1vw'}
    ),

    # Footer
    html.Footer(
        ["Copyright MK, My  ",
         html.A("personal site", href="https://karamelih.github.io"),],
        style={'margin-top': 'auto', 'text-align': 'center', 'color': 'gray', 'font_size': '22px'}
    )
], style={'display': 'flex', 'minHeight': '100vh', 'backgroundColor': 'black'},)
...
if __name__ == '__main__':
    try:
        ...
        print(f"{e} try setting the LOCAL argument to True in the fl_app.py file")
